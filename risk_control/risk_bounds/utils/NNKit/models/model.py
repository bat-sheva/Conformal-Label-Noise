...
class vanilla_nn(nn.Module):
    def __init__(self, input_size=1, output_size=1, bias=True,
        ...
        self.fcs = torch.nn.Sequential(*self.fcs)
...
class binary_nn(vanilla_nn):
    def __init__(self, input_size=1, output_size=1, bias=True,
                 hidden_size=400, num_layers=4,
                 use_bn=False, actv_type='relu',
                 softmax=False, dropout=0):
        super(binary_nn, self).__init__(input_size, output_size, bias, hidden_size, num_layers,
                                  use_bn, actv_type, softmax, dropout)

    def forward(self, X):
        X = super().forward(X)
        X = (torch.tanh(2 * X) + 1) / 2
        return X.squeeze()


class selective_net_nn(nn.Module):
    def __init__(self, input_size=1, y_size=1, bias=True,
                 hidden_size=400, num_layers=4,
                 use_bn=False, actv_type='relu', dropout=0):
        ...
        for _ in range(num_layers - 1):
            self.fcs.append(LinearLayer(hidden_size, hidden_size, bias,
                                        use_bn=use_bn, actv_type=actv_type, dropout=dropout))
        self.fcs.append(LinearLayer(hidden_size, y_size * 3, bias,
                                    use_bn=False, actv_type=None, dropout=0))
...
class binary_classification_nn(nn.Module):
    def __init__(self, input_size=1, bias=True,
                 hidden_size=400, num_layers=4,
                 use_bn=False, actv_type='relu', dropout=0):
        ...
        for _ in range(num_layers - 1):
            self.fcs.append(LinearLayer(hidden_size, hidden_size, bias,
                                        use_bn=use_bn, actv_type=actv_type, dropout=dropout))
        self.fcs.append(LinearLayer(hidden_size, 1, bias,
                                    use_bn=False, actv_type=None, dropout=0))
