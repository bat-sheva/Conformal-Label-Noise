...
def args_to_txt(args):
    args_summary = f'{args.method_name}'
    return args_summary
...
def weighted_pearsons_corr(x, y, weights):
    m_x = weights @ x / weights.sum()
    m_y = weights @ y / weights.sum()

    v_x = x - m_x
    v_y = y - m_y

    s_x = weights @ (v_x ** 2) / weights.sum()
    s_y = weights @ (v_y ** 2) / weights.sum()
    s_xy = weights @ (v_x * v_y) / weights.sum()
    return s_xy / torch.sqrt(s_x * s_y)
...
def chi_squared_test(y, y_lower, y_upper):
    ...
    interval_sizes = y_upper - y_lower
...
def wsc(X, y, y_upper, y_lower, nominal_coverage_level, delta=0.1, M=1000, verbose=False, random_state=2021):
    assert 0 <= nominal_coverage_level <= 1
    ...
    def wsc_v(X, y, y_upper, y_lower, delta, v):
        ...
        bi_best = n
        # cover_min = 1
        cover_best = nominal_coverage_level
        for ai in np.arange(0, ai_max):
            ...
            if abs(cover_star - nominal_coverage_level) > abs(cover_best - nominal_coverage_level):
                ai_best = ai
                bi_best = bi_star
                cover_best = cover_star
        return cover_best, z_sorted[ai_best], z_sorted[bi_best]
    ...
    idx_star = np.argmax(abs(np.array(wsc_list) - nominal_coverage_level))
...
def wsc_unbiased(X, y, y_upper, y_lower, nominal_coverage_level, delta=0.1, M=1000, test_size=0.6, random_state=2021,
                 verbose=False):
    ...
    wsc_star, v_star, a_star, b_star = wsc(X_train, y_train, y_upper_train, y_lower_train,
                                           nominal_coverage_level=nominal_coverage_level,
                                           delta=delta, M=M,
                                           verbose=verbose,
                                           random_state=random_state)
    # Estimate coverage
