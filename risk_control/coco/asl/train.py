...
parser.add_argument('--model-path', default='./Open_ImagesV6_TRresNet_L_448.pth', type=str)
...
def train_multi_label_coco(model, train_loader, val_loader, lr, args):
    ...
    trainInfoList = []
    scaler = GradScaler()
    save_dir = os.path.join('models', f'noise_type={args.noise_type}_noise_level={args.noise_level}')
    ...
    for epoch in (range(Epochs)):
        ...
        #         os.mkdir(save_dir)
        #     torch.save(model.state_dict(), os.path.join(save_dir, 'model-{}-{}.ckpt'.format(epoch + 1, i + 1)))
        # except Exception as e:
        #     print(f"warning: failed saving epoch {epoch} model at path {save_dir} because: {e}")
        ...
        if mAP_score > highest_mAP:
            ...
            try:
                torch.save(model.state_dict(), os.path.join(save_dir, 'model-highest.ckpt'))
            except Exception as e:
                print(f"warning: failed saving best model epoch {epoch} at path {save_dir} because: {e}")
