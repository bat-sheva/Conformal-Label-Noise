...
from src.helper_functions.helper_functions import mAP, CocoDetection, CutoutPIL, ModelEma, add_weight_decay, \
    CIFARDetection, set_seeds, get_seed
...
def main():
    ...
    images = train_dataset.data[idx]
    train_dataset = CIFARDetection(images[:45000], labels[:45000], transform=train_transform)
    val_dataset = CIFARDetection(images[45000:], labels[45000:], transform=transform)
    set_seeds(seed)
...
def train_multi_label_coco(model, train_loader, val_loader, lr, args):
    ...
    for epoch in (range(Epochs)):
        ...
        #         os.mkdir("models/cifar10")
        #     torch.save(model.state_dict(), os.path.join(
        #         f'models/cifar10', 'model-{}-{}.ckpt'.format(epoch + 1, i + 1)))
        # except:
        #     pass
        ...
        if mAP_score > highest_mAP:
            ...
            try:
                torch.save(model.state_dict(), os.path.join(
                    f'models/cifar10', 'model-highest.ckpt'))
            except Exception as e:
                print(f"warning: failed saving best model epoch {epoch} because: {e}")
