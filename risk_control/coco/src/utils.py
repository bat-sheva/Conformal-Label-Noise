...
def get_scores_targets(model, loader, corr, n_labels, compute_scores):
    scores = torch.zeros((len(loader.dataset), n_labels))
    labels = torch.zeros((len(loader.dataset), n_labels))
    i = 0
    ...
    with torch.no_grad():
        for x, label in tqdm(loader):
            ...
            batch_labels = torch.zeros((x.shape[0], n_labels))
            labels_idx = label.sum(dim=1).nonzero().squeeze()
            batch_labels[labels_idx[:, 0], labels_idx[:, 1]] = 1
            # for j in range(x.shape[0]):
            #     batch_labels[j, label[0].sum(dim=0).nonzero()[:, 0]] = 1
            # for j in range(len(annotations)):
            #     for annotation in loader.dataset.coco.loadAnns(annotations[j]):
            #         batch_labels[j,corr[annotation['category_id']]] = 1 label[0].sum(dim=0).nonzero()[:, 0]  label[j].nonzero()[:, 1]
