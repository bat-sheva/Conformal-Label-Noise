...
def get_data(cache_path, noise_level, noise_type, is_clean, model_trained_on_noisy):
    cache_path = os.path.join(cache_path, is_clean)
    if model_trained_on_noisy:
        model_path = f'./PraNet/snapshots/PraNet_Res2Net/noise_type={noise_type}_noise_level={noise_level}PraNet-19.pth'
    else:
        model_path = f'./PraNet/snapshots/PraNet_Res2Net/PraNet-19.pth'

    test_size = 352
    T = 10 
    folders = [ 'CVC-300', 'CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB'] # TODO: deleted 'HyperKvasir'
            ...
            for i in range(test_loader.size):
                ...
                if not is_clean:
                    ...
                    if noise_type == 'uniform':
                        noise_level_matrix = torch.ones_like(temp_mask) * noise_level  # TODO: add noise
                        noise_matrix = (torch.rand_like(noise_level_matrix) <= noise_level_matrix).int()
                        noisy_gt = noise_matrix * (1 - temp_mask) + (1 - noise_matrix) * temp_mask
                    elif noise_type == 'nonuniform':
                        noise_level_matrix = torch.ones_like(temp_mask) * noise_level
                        noise_matrix = (torch.rand_like(noise_level_matrix) <= noise_level_matrix).int()
                        noise_matrix[..., 50:80, 50:80] = noise_matrix[..., 50, 50].item()
                        # noise_matrix[..., 150:240, 180:270] = noise_matrix[..., 150, 180].item()
                        noise_matrix[..., 220:300, 300:320] = noise_matrix[..., 250, 300].item()
                        noisy_gt = noise_matrix * (1 - temp_mask) + (1 - noise_matrix) * temp_mask
                    elif noise_type == 'nonuniform2':
                        noise_matrix = torch.zeros_like(temp_mask).int()
                        noise_matrix[..., 120:250, 120:250] = (torch.rand(1) <= noise_level).int().item()
                        noisy_gt = noise_matrix * (1 - temp_mask) + (1 - noise_matrix) * temp_mask
                    temp_mask = noisy_gt.numpy()
                ...
                temp_mask = gaussian(temp_mask, sigma=1)  # TODO: there was multichannel=True
